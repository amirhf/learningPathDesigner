...
func CreatePlan(cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		plannerURL := fmt.Sprintf("%s/plan", cfg.PlannerServiceURL)
		...
		// Send request
		client := &http.Client{
			Timeout: 60 * time.Second, // Longer timeout for plan generation
		}
